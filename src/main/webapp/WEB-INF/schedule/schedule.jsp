...
<%
		...
		while(cal.get(Calendar.MONTH) == mm) {
			String ymd = cal.get(Calendar.YEAR)+"-"+(cal.get(Calendar.MONTH)+1)+"-"+cal.get(Calendar.DATE);  // 화면에 표시되는 일자(년월일)를 ymd변수로 셋팅
			if(cal.get(Calendar.DAY_OF_WEEK) == 1) {
				out.println("<td><a href='scContent.sc?ymd="+ymd+"&mid=${sMid}'><font color='red'>"+cal.get(Calendar.DATE)+"</font></a></td>");
			}
			else if(cal.get(Calendar.DAY_OF_WEEK) == 7) {
				out.println("<td><a href='scContent.sc?ymd="+ymd+"&mid=${sMid}'><font color='blue'>"+cal.get(Calendar.DATE)+"</font></a></td>");
			}
				...
				out.println("<td style='background-color:skyblue'><a href='scContent.sc?ymd="+ymd+"&mid=${sMid}'><b>"+cal.get(Calendar.DATE)+"</b></a></td>");
			}
			else {
				out.println("<td><a href='scContent.sc?ymd="+ymd+"&mid=${sMid}'><font color='#000'>"+cal.get(Calendar.DATE)+"</font></a></td>");
			}
